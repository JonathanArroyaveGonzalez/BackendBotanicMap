...
def read_fauna(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    fauna = crud.get_fauna(db, skip=skip, limit=limit)
    return fauna

@router.get("/getFaunaById/{fauna_id}", response_model=schemas.Fauna)
def read_fauna_by_id(fauna_id: int, db: Session = Depends(get_db)):
...
def create_fauna(fauna: schemas.FaunaCreate, db: Session = Depends(get_db)):
    return crud.create_fauna(db=db, fauna=fauna)

@router.delete("/deleteFaunaById/{fauna_id}")
def delete_fauna(fauna_id: int, db: Session = Depends(get_db)):
